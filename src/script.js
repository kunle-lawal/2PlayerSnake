    ...
    firebase.auth().onAuthStateChanged(firebaseUser => {
        ...
        if(!firebaseUser) {
            auth.signInAnonymously().then((data) => {
                db.collection('players').doc(data.user.uid).set({
                    snake: game.snake
                })
            });
        } else {
            playerRef = db.collection('players').doc(auth.currentUser.uid);
        }
    })
    ...
    let game = {
        ...
        drawSnake() {
            for (let i = 0; i < this.snake.tails.length; i++) {
                ...
                ctx.fillRect(tail.x, tail.y, this.snake.w, this.snake.h)
            }
        },

        updateSnake() {
            let snake = this.snake;
            this.moveSnake('snake', snake.direction.current)
            ...
            if (this.checkCollision(snake, snake.tails)) {
                $('#player1Info #heart' + snake.hearts).removeClass('full');
                snake.hearts -= 1;
            }
            ...
            if (snake.tails[0].x === this.food.x &&
                snake.tails[0].y === this.food.y) {
                    ...
                    // wait = window.setTimeout(function() {
                        ...
                        // console.log(snake)
                document.getElementById('score1').innerHTML = snake.total
                return true;
            }
        },
        ...
        changeDirection(snake, newDirection) {
            ...
            this.snake.direction[this.snake.direction.current] = false;
            this.snake.direction[newDirection] = true;
            this.snake.direction.current = newDirection;
            playerRef.set({
                snake: {
